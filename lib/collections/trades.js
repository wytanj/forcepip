Trades = new Mongo.Collection('Trades');

var tradeCounter = Trades.count();

Trades.allow({
  update: function(userId, trade) { return hasPermission(userId, post); },
  remove: function(userId, trade) { return hasPermission(userId, post); },
});

Trades.deny({
  update: function(userId, trade, tradeAttributes) {
    // may only edit the following two fields:
    return (_.without(tradeAttributes, 'pair', 'trigger').length > 0);
  }
});

validateTrade = function (tradeAttributes) {
  var errors = {};

  if (!tradeAttributes.body)
    errors.body = "Please fill in a headline";

  return errors;
};

Meteor.methods({
  tradeInsert: function(tradeAttributes) {
    check(this.userId, String);
    check(tradeAttributes, {
      tradeId: String,
      body: String
    });

    var user = Meteor.user();
    var tradeId = this._id;
    // var trade = Trades.findOne(tradeAttributes.tradeId);

    var trade = _.extend(tradeAttributes, {
      tradeId: tradeId,
      author: user.username,
      submitted: new Date(),
      commentsCount: 0,
      upvoters: [],
      votes: 0
    });

    var otherAttributes = {
      pair: 'EUR/USD',
      state: 1, // 1= upcoming, 2 = live, 3 = past
      buy: true,
      stopLoss: 1.1140,
      takeProfit: 1.4444,
      tradeCount: tradeCounter + 1,
      triggerPrice: 0
    };
    //
    // EDIT OLD Trade, EUR/USD, Buy @ 1.0648, SL @ 1.0920, TP @ 1.1148, Trade 192/2015.New Trade, Oil, Buy Stop @ 36.90. TP @ 49.90. Sell Stop (act as SL) @ 32.40, Small Lot.. Trade 194/2015

    }
});
